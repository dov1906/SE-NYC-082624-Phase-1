...
// burgers.forEach(burger => console.log(burger.name))

// burgers.find(burger => burger.name === "Flatburger")

// burgers.find(burger =>{ 
//     return burger.name[0] === "m"

// })



burgers.forEach(burger => burger.description)

for(burger of burgers){
    const brg = document.createElement('img')
    brg.className = `${burger.name}`
    brg.src = burger.image
    document.getElementById('restaurant-menu').appendChild(brg)

}


otherFoods.forEach(burger => {
    const othr = document.createElement('img')
    othr.src = burger.image.toString()
    restaurant-menu.appendChild(othr)
...
element.textcontent = burgers[0].name
